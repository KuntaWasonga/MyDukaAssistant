...
def get_uuid_from_integer(integer_value):
    ...
    bytes_value = integer_value.to_bytes(
        (integer_value.bit_length() + 7) // 8, 'big')
...
@employee_bp.route("/employee/current", methods=['GET'])
...
def delete(id):
    ...
    if mtu.admin:
        ...
        if employee:
            ...
            db.session.commit()
            return jsonify({'message': f'{employee.employee_id} terminated'}), 200
